...
# FIXME: 把 action 写进 memory token
class ActionConditionedEncoder(nn.Module):
    """
    动作条件编码器
    该模块将动作信息与token特征相结合，通过一个简单的神经网络将动作条件融入到token表示中。
    适用于需要根据动作信息调整token表示的场景。
    参数:
        token_dim (int): token特征的维度
        action_dim (int): 动作向量的维度
    """
    def __init__(self, token_dim, action_dim):
        super().__init__()
        self.proj = nn.Sequential(
            nn.Linear(token_dim + action_dim, token_dim),
            nn.GELU(),
            nn.Linear(token_dim, token_dim),
        )
    def forward(self, token, action):
        """
        前向传播函数
        参数:
            token (torch.Tensor): token特征，形状为[N, D]，其中N是token数量，D是token维度
            action (torch.Tensor): 动作向量，形状为[A]，其中A是动作维度
        返回:
            torch.Tensor: 融合了动作信息的token特征，形状与输入token相同[N, D]
        """
        # 将动作向量扩展到与token相同的第一维度
        action_expand = action.unsqueeze(0).expand(token.size(0), -1)
        # 拼接token和动作特征，并通过投影网络
        return self.proj(torch.cat([token, action_expand], dim=-1))
