...
def make_interleaved_dataset(
    dataset_kwargs_list: List[Dict],           # 数据集配置参数列表，每个元素都会传递给 [make_dataset_from_rlds](file://d:\doctor-code\code\MemoryVLA-openvla-codebase\MemoryVLA-openvla-codebase\vla\datasets\rlds\dataset.py#L37-L252)
    sample_weights: Optional[List[float]] = None,  # 每个数据集的采样权重，如果为None则默认均匀采样
    *,
    train: bool,                               # 是否为训练数据集
    shuffle_buffer_size: int,                  # 数据集洗牌缓冲区大小（以帧数计）
    traj_transform_kwargs: Optional[Dict] = None,  # 传递给 [apply_trajectory_transforms](file://d:\doctor-code\code\MemoryVLA-openvla-codebase\MemoryVLA-openvla-codebase\vla\datasets\rlds\dataset.py#L255-L355) 的参数
    frame_transform_kwargs: Optional[Dict] = None, # 传递给 [apply_frame_transforms](file://d:\doctor-code\code\MemoryVLA-openvla-codebase\MemoryVLA-openvla-codebase\vla\datasets\rlds\dataset.py#L373-L428) 的参数
    batch_size: Optional[int] = None,          # 批次大小，如果不提供则不进行批处理
    balance_weights: bool = False,             # 如果为True，采样权重会乘以每个数据集的帧数
    traj_transform_threads: Optional[int] = None,  # 轨迹变换的总并行调用数，根据采样权重分配到各数据集
    traj_read_threads: Optional[int] = None,   # 轨迹读取的总并行工作线程数，根据采样权重分配到各数据集
    load_all_data_for_training: bool = True,   # 是否加载所有数据用于训练
) -> dl.DLataset:
    """
    从数据集配置列表创建一个交错数据集。返回一个批次化的帧数据集。

    Args:
        dataset_kwargs_list: 参数列表，每个元素都传递给 [make_dataset_from_rlds](file://d:\doctor-code\code\MemoryVLA-openvla-codebase\MemoryVLA-openvla-codebase\vla\datasets\rlds\dataset.py#L37-L252)。
            "num_parallel_calls" 和 "num_parallel_reads" 会被 `traj_transform_threads` 和
            `traj_read_threads` 覆盖。
        sample_weights: 列表中每个数据集的采样权重。如果为None，默认为均匀分布。
        train: 指定这是训练数据集还是验证数据集。
        shuffle_buffer_size: 数据集洗牌缓冲区大小（以帧数计算）。
        traj_transform_kwargs: 传递给 [apply_trajectory_transforms](file://d:\doctor-code\code\MemoryVLA-openvla-codebase\MemoryVLA-openvla-codebase\vla\datasets\rlds\dataset.py#L255-L355) 的参数。"num_parallel_calls" 会被
            `traj_transform_threads` 覆盖。
        frame_transform_kwargs: 传递给 [apply_frame_transforms](file://d:\doctor-code\code\MemoryVLA-openvla-codebase\MemoryVLA-openvla-codebase\vla\datasets\rlds\dataset.py#L373-L428) 的参数。
        batch_size: 批次大小，如果未提供则输出不会被批处理。
        balance_weights: 如果为True，采样权重会乘以每个数据集中的帧数。
            这使得当所有采样权重相等时，遍历交错数据集一次相当于遍历每个单独的数据集一次（期望上成立，
            实际上由于采样是随机的所以只是期望值）。
        traj_transform_threads: 轨迹变换的总并行调用数，根据采样权重分布在各个数据集中。
            如果为None，则每个数据集默认为AUTOTUNE。
        traj_read_threads: 轨迹读取的总并行工作线程数，根据采样权重分布在各个数据集中。
            如果为None，则每个数据集默认为AUTOTUNE。
    """
    # 如果未指定 `sample_weights`，则默认均匀采样
    ...
    # 检查有效的 `traj_transform_kwargs` 和 `frame_transform_kwargs`
    ...
    # 获取"主要"数据集的索引（即 sample_weight == 1.0 的数据集）
    ...
    # 有效数据集长度 = 每个数据集至少完成一个周期所需的样本数
    # 注意：只计算"主要"数据集（即 sample_weight == 1.0 的数据集）
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        dataset = apply_trajectory_transforms(
            dataset.repeat(),  # 重复数据集
            **traj_transform_kwargs,
            num_parallel_calls=threads,
            dataset_statistics=all_dataset_statistics[dataset_kwargs["name"]],
            train=train,
        ).flatten(num_parallel_calls=threads)  # 展平轨迹为帧
    ...
    # 验证 =>> 固定一个洗牌缓冲区的数据并缓存在RAM中；防止内存逐渐增加！
    if not train:
        dataset = dataset.take(shuffle_buffer_size).cache()

    # 洗牌数据集
    # 重要 :: 在 .cache() 之后进行洗牌，否则内存仍会泄漏！
